<?php   										// Opening PHP tag
	...
	function orderInfo (PDO $pdo, string $email, string $orderNum){
		$sql = "SELECT customer.cname AS name, customer.username AS username, orders.ordernum AS ordernum, orders.quantity AS quantity, orders.date_ordered as date_ordered, orders.date_deliv AS date_deliv
		FROM
			customer JOIN orders
		ON
			customer.custnum = orders.custnum
		WHERE
			orders.ordernum = :orderNum AND customer.email = :email";
		// Execute the SQL query using the pdo function and fetch the result
		$order = pdo($pdo, $sql, ['orderNum' => $orderNum, 'email' => $email])->fetch();
